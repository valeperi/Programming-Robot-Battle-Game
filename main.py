...
class Robot(turtle.Turtle): # define the object in the game, we make it inherite from the turtle class
    def __init__(self, spriteshape, color, startx, starty):
        ...
        self.speed = 1
        self.shapesize(stretch_wid = 1.5, stretch_len = 1, outline = None) # size of the sprite in terms of width and length (1,1 is the basic one)
        self.speed = 20
    ...
    def move(self,deg,game):
        if self.state == "Playing" and game.GameState == "GameRunning":
            ...
            if self.xcor() > 285:
                self.setx(285) # set the x position
                self.rt(deg) # set the right rotation angle
            if self.xcor() < - 285:
                self.setx(-285)
                self.rt(deg)
            if self.ycor() > 285:
                self.sety(285) # set the y position
                self.rt(deg) # set the right rotation angle
            if self.ycor() < -285:
                self.sety(-285)
                self.rt(deg)
